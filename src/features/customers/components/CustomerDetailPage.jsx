...
import { useBranchesList } from "@/features/branches/queries/branchQueries";
import { useContractMutations } from "@/features/contracts/queries/contractQueries";
import {
  ...
  useCustomersList
} from "@/features/customers/queries/customerQueries";
...
export default function CustomerDetail({ isSplitView = false }) {
  ...
  const { data: allCustomers = [] } = useCustomersList({ enabled: isNew });
  ...
  const { deleteContract } = useContractMutations(customerId);
  ...
  const completeDraftMutation = useMutation({
    mutationFn: () => {
      return updateCustomer.mutateAsync({ customerId, payload: {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => deleteCustomer.mutateAsync(customerId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['customers'] });
  ...
  const deleteContractMutation = useMutation({
    mutationFn: (contractId) => deleteContract.mutateAsync(contractId),
    onSuccess: () => {
